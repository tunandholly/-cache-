...
void cache_sim_t::init()
{
  ...
  fifo_queues = new std::queue<size_t>[sets]; // initialize FIFO queue
  read_accesses = 0;
  read_misses = 0;
  bytes_read = 0;
  write_accesses = 0;
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  // if FIFO queue not full，choose next way
    if (fifo_queues[idx].size() < ways)
    {
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
  ...
  {
    //FIFO replacement
    uint64_t victim_addr = fifo_queue.front();
    fifo_queue.pop();
    old_tag = tags[victim_addr];
    tags.erase(victim_addr);
    
  }
  tags[addr >> idx_shift] = (addr >> idx_shift) | VALID;
  fifo_queue.push(addr >> idx_shift); // add into FIFO queue
  return old_tag;
}
